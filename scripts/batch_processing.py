...
def run_batch_processing():
    ...
    # Step 0: Check for human responses and stop automation
    logger.info("\nStep 0: Checking for human responses...")
    stopped_count = check_and_stop_for_responses(sheets_client, workflow_sheet_id)
    logger.info(f"Automation stopped for {stopped_count} leads due to human responses")
    ...
    for i, lead in enumerate(leads_needing_followup, 1):
        # Check if automation should be stopped for this lead
        if should_stop_automation(sheets_client, workflow_sheet_id, lead['lead_id']):
            logger.info(f"Skipping {lead['lead_id']} - automation stopped due to human response")
            continue
    ...
    for i, lead in enumerate(leads_needing_connection, 1):
        # Check if automation should be stopped for this lead
        if should_stop_automation(sheets_client, workflow_sheet_id, lead['name']):
            logger.info(f"Skipping {lead['name']} - automation stopped due to human response")
            continue
    ...
    logger.info("=" * 60)
    logger.info(f"Daily message limit: {DAILY_MESSAGE_LIMIT}")
    logger.info(f"Leads with human responses: {stopped_count}")
    logger.info(f"Follow-up messages needed: {followup_count}")
    ...
    try:
        ...
        for i, header in enumerate(headers):
            ...
            if 'automation stopped' in header_lower or 'manual stop' in header_lower:
                ...
                stop_reason_col = i
            elif 'last action date' in header.lower() or 'last updated' in header.lower():
                last_action_date_col = i
